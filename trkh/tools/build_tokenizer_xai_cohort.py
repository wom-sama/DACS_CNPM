...
def build_cohort(
    *,
    ...
    output_dir: Path,
    max_cases: int = 16,
    mode: str = "validation_only_inceptionnext_atto_changed_case_cohort",
) -> Dict[str, object]:
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a deterministic validation-only tokenizer changed-case XAI cohort."
    )
    ...
    parser.add_argument(
        "--mode",
        default="validation_only_inceptionnext_atto_changed_case_cohort",
    )
