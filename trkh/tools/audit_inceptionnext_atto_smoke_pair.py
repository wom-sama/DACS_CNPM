...
EXPECTED_METHOD = "inceptionnext_atto_surface_tokenizer"


def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Locked full-validation comparison for the matched InceptionNeXt-Atto tokenizer smoke."
    )
...
def _validate_provenance(
    *,
    ...
    if stage_a.get("method") != EXPECTED_METHOD:
        raise ValueError("Stage-A method does not match InceptionNeXt-Atto")
    ...
    expected_stems = ((control_config, "conv_pool"), (candidate_config, EXPECTED_METHOD))
    for config, expected_stem in expected_stems:
        ...
        candidate_stem = "inceptionnext_atto_tokenizer" if expected_stem == EXPECTED_METHOD else expected_stem
        if observed_stem != candidate_stem:
