...
| `B46-01` | `PAUSED_UNSCORED` | B43 failed because activation matching never constrained the classifier boundary; one graph may instead learn clean-control logits and relit-robust logits under explicit class-1 margin protection. | All metric-free gates pass: robust target loss falls to `0.0796x`, clean leakage is `0.0863x`, all ten clean argmaxes remain exact, target geometry/margins pass and peak VRAM is `0.991 GiB`. Gradient mass spans patch embed and all 12 blocks, agreeing with B44's diffuse correction. Preflight SHA256 `80fd9071395cce798bd53c5fc8a04668cd5f3ee927a74eee3b454c34ef86d424`. The authorized formal run completed both matched nine-epoch endpoint checkpoints, then was interrupted before `b46_pareto_margin.safetensors`, `held_condition_scores.npz` or `summary.json` existed; therefore it produced no reportable candidate or gate result. | Pause the local pretrained direction at the user's request. Preserve the committed implementation and accepted preflight, but do not infer a pass/fail result, resume from the incomplete directory, or open validation/test. Any future restart must use a new output directory and the unchanged preregistered fold-0 protocol. |
...
B37 completed on clean commit `4d2076e`; all three balanced readouts converged
(`1344/2028/811` iterations for patch/official/deranged). The aligned CLS signal
is real relative to derangement, but is not conditionally useful beyond the
strong patch representation.

| Held TRAIN fold 1, clean | Accuracy | Macro-F1 | C1 P/R/F1 | C1 TP | Restricted FP | Pair AUROC |
|---|---:|---:|---:|---:|---:|---:|
| Native B35 head | `0.942149` | `0.915462` | `0.840909/0.725490/0.778947` | `74` | `14` | `0.984642` |
| Patch-only balanced readout | `0.926800` | `0.899405` | `0.789474/0.735294/0.761421` | `75` | `20` | `0.977547` |
| Official CLS + patch readout | `0.925620` | `0.897535` | `0.754902/0.754902/0.754902` | `77` | `24` | `0.976250` |
| Deranged-CLS + patch control | `0.923259` | `0.891300` | `0.769231/0.686275/0.725389` | `70` | `21` | `0.978470` |

Official concatenation adds three TP versus native but doubles the extra errors:
ten more restricted FP, including `0->1: 8 -> 14`, and loses accuracy, macro-F1
and ranking. Under bright exposure C1 F1 changes `0.595041 -> 0.560000` and
`0->1` changes `58 -> 65`. Nine of eleven gates fail. The likely mechanism is
stage mismatch: frozen raw DINO exposes useful global CLS information, whereas
B35 supervised only an average-patch head and therefore did not preserve CLS as
an independently calibrated task endpoint. Do not implement or tune the simple
dual head.

- Summary SHA256 `57ee7b42030fa60988493dd64c00c1a12432fdf65628d45570a8add631fa8c9c`:
...
## Closed experiment: B38 nuisance-conditioned BiCAR
...
The formal TRAIN component-fold-2 run closed the exact objective. Control and
candidate initial state, both warm-up epoch losses, optimizer updates, learning
rates, update ratios, model hashes and EMA hashes replay exactly. The candidate
uses all `2,961` expected auxiliary calls; task/auxiliary gradients remain
positively aligned by role, and peak CUDA allocation is only `2.233 GiB` versus
`1.384 GiB` control. Thus the failure is neither RNG drift, branch starvation,
gradient conflict at the coarse role level nor an 8-GB resource limit.

| Condition | Control C1 F1 | Candidate C1 F1 | Delta | Main transition |
|---|---:|---:|---:|---:|
| clean | `0.711111` | `0.703911` | `-0.007200` | TP `64 -> 63`; restricted FP `21 -> 21` |
| dim | `0.641509` | `0.641509` | `0.000000` | `2 -> 1: 44 -> 45` |
| bright | `0.465116` | `0.593301` | `+0.128185` | `0 -> 1: 94 -> 38` |
...
- Formal summary:
  `runs/b38_relit_bicar_fold2_2f9a008_20260809/summary.json`, SHA256
  `ac7d33a7c2b893c83a6ece8d643ee528d4411baa7bae94e127946080cca984f5`.
- Held scores SHA256
  `b1fb146c92d1a1d95164beb63ee220a6468e47cdf3e39505e126c218d6e28b8d`.
- Preflight SHA256
  `36b62abf277112ac7f8d36e6c779def7c2d1de96bfa23cd7a68a86994319f47f`.
...
## Closed preflight: B39 stratified pair BiCAR with fixed midpoint
...
B39 stopped at its committed preflight on `dddaddd`; no training arm, held-fold
metric, validation dataset or test dataset was constructed. State load, all-class
batch membership, CPU/CUDA RNG restoration, finite losses, both EMA updates,
clean-class-1 compatibility, pair balance and self-midpoint identity all pass.
The two decisive failures are structural:

- weighted auxiliary/task gradient norm is `0.338169` in the backbone and
  `0.285357` in the head, above the locked `0.15` ceiling despite positive
  cosines `0.243033/0.423320`;
- for natural pair counts `(1572,399)`, the bright `0<->1` oracle produces
  gradients `[[-0.076700,+0.076700],[0,0]]`, so the true-class-1 direction is
  unpenalized.

The second result is not a data accident. For the 2x2 off-diagonal matrix
`[[0,a],[b,0]]`, singular values are `|a|` and `|b|`; its spectral norm is their
maximum and therefore has zero derivative with respect to the smaller direction
away from a tie. Frequency-symmetric left/right scaling multiplies both pair
directions by the same product and cannot remove that max-mode behavior. Exact
artifact
`runs/b39_stratified_pair_preflight_dddaddd_20260809/preflight.json` has SHA256
`ffaa1c822fdc768c91e907cd62811d335456bdbaa6c9da0068b766ade7ab5597`.
B39 will not be rescaled or rerun. Its failure narrows the successor to a
two-direction monotone confusion functional whose scale is locked before any
held-fold evaluation.

## Closed preflight: B40 bidirectional pair-confusion mean

B40 changes the failed B39 auxiliary functional, not the model architecture,
data, endpoint interpolation or robustness conditions. For a relit two-class
pair `(p,q)`, CAR's differentiable matrix already averages each column over rows
whose true class is that column. The proposed working functional therefore uses
both conditional directions explicitly:

```text
M_pair = 0.5 * (C_hat[pred=p,true=q] + C_hat[pred=q,true=p])
L_aux  = 0.5 * M_bright(0,1) + 0.5 * M_dim(1,2)
L_raw  = L_task + 0.15 * L_aux
theta_gate = 0.5 * EMA(theta_control) + 0.5 * EMA(theta_raw)
```

This is called bidirectional pair-confusion mean here; it must not be presented
as an established new method until repeated evidence exists. Unlike the 2x2
spectral maximum, decreasing either off-diagonal term strictly decreases the
pair functional and both unequal oracle directions receive nonzero gradients.
Because each column is already a per-true-class mean, dataset-frequency factors
would only multiply both directions of a pair by the same product; B40 omits
that redundant scale. Confusion-matrix optimization is a recognized way to
retain error-type information under imbalance
([Koço and Capponi, ACML 2013](https://proceedings.mlr.press/v29/Koco13.html)),
and multiclass metric theory explicitly represents performance through the
off-diagonal confusion vector
([Hiranandani et al., NeurIPS 2019](https://proceedings.neurips.cc/paper/2019/hash/1fd09c5f59a8ff35d499c0ee25a1d47e-Abstract.html)).
Those sources motivate confusion-aware objectives; the two-direction mean and
its TRKH evidence remain this project's proposal.

The outer coefficient is not selected from accuracy/F1. B39 used `0.5` and its
one-batch weighted auxiliary/task gradient ratios were `0.3382/0.2854`; applying
the single mechanical factor `0.30` gives the locked coefficient `0.15` and a
predicted scale near `0.10/0.086`. The new vector direction can change those
ratios, so B40 still fails closed unless an exact mature-model preflight observes
`[0.04,0.16]` for backbone and head with task cosine `>=0`. Against clean
class-1 task loss, ratio must be `[0.02,0.20]` and cosine `>=0.20`. Dim/bright
gradient ratio must be `[0.50,2.0]` with cosine `>=-0.50`; an unequal-error
oracle must give the correct nonzero direction to every pair row with minimum
row-gradient norm at least `0.10` of the maximum. Exact B38 mature state, both
pair EMA updates, positive directional entries, CPU/CUDA RNG restoration and
self-midpoint identity are also mandatory.

B39 never trained or evaluated held component fold 3; its preflight used one
batch from the prospective fit partition only. B40 may therefore use the same
still-unseen held TRAIN fold if preflight passes. Formal control/candidate retain
seed `42`, tempered sampling, LDAM-Focal, DINOv3-S/16 topology, auxiliary start
epoch 3, nine epochs on the 30-epoch horizon, luminance-only `+/-0.25` brightness
with `0.10` contrast, and separate pair EMAs at momentum `0.5`. The endpoint
gate is unchanged from B39/B38: clean C1 `>=+0.005`; macro/accuracy/pair deltas
`>=-0.002/-0.002/-0.003`; no clean TP loss or restricted-FP increase; dim,
bright and mean robust C1 `>=+0.010/+0.015/+0.015`; bright `0->1` reduction
`>=10%`. Raw candidate remains mechanism evidence. The midpoint adds no
inference parameter, operator or latency; mobile/INT8 claims remain forbidden
until scientific promotion and physical-device audits.

B40 stopped on clean commit `b9afb4f` before either training arm. The proposed
functional resolves B39's mathematical defect: all six unequal-error oracle
checks pass with minimum/maximum row-gradient ratio `0.220148`, and actual
bright/dim directional EMA entries are all positive
(`0.171875/0.229818/0.164714/0.205566`). State, class coverage, RNG restoration,
EMA updates, midpoint identity and bright/dim balance also pass.

The failure localizes to gradient routing. Weighted auxiliary/task ratios are
`0.038844` backbone and `0.022544` head, below the locked `0.04` floor. More
importantly, the same auxiliary is compatible with the full clean task in the
backbone (`cos=+0.201112`) but conflicts in the classifier head
(`cos=-0.051550`). Against clean class-1 loss, backbone/head cosines are both
positive (`+0.403319/+0.264046`), although the head ratio is only `0.016201`.
Thus increasing B40's scalar would strengthen a measured full-task head
conflict; it is not an authorized repair.

Exact artifact
`runs/b40_pair_mean_preflight_b9afb4f_20260809/preflight.json` has SHA256
`6f4f64d0f284f17f2bded5dba63529d395496f62a9be6a7394f906dbc1555480`.
It reports `train=false`, `validation=false`, `test=false`; no held-fold score
exists. B40 is closed without a rerun. The only evidence-supported continuation
is representation-only auxiliary routing: preserve the classifier as a clean
task endpoint while allowing the compatible robust gradient through its fixed
...
## B41 closure: routing works, arithmetic weight averaging does not

The committed TRAIN-only preflight passed every locked check. At auxiliary
weight `0.20`, backbone task/auxiliary ratio and cosine are
`0.051614/+0.201446`; the clean-class-1 values are
`0.029729/+0.400781`; dim/bright auxiliary ratio and cosine are
`1.212253/+0.122463`. Clean-task head norm remains nonzero while every
auxiliary head norm is exactly zero. The accepted preflight is
`runs/b41_backbone_routed_preflight_fbea170_20260809/preflight.json`, SHA256
`2b5b06dea14343ea1225f51a849de8993034045927cf362e432adf7fd8b0a43f`.

The matched fold-3 run completed all `1242` updates per arm with exact warm-up
and checkpoint replay. Control versus raw routed endpoint is:

| Condition | Control C1 F1 | Raw C1 F1 | Raw delta | Key confusion change |
|---|---:|---:|---:|---|
| clean | `0.736842` | `0.727273` | `-0.009569` | TP `77 -> 76`, restricted FP `34 -> 35` |
| dim | `0.663900` | `0.677686` | `+0.013786` | TP `80 -> 82`; `0->1` `8 -> 5` |
| bright | `0.501818` | `0.553719` | `+0.051901` | restricted FP `108 -> 77`; `0->1` `95 -> 64` |

This proves that the routed objective learns the intended lighting correction,
but the raw endpoint pays a small clean cost. The prospectively gated fixed
weight midpoint fails: clean accuracy/macro/C1/pair deltas are
`-0.000604/-0.002037/-0.006984/-0.000754`, clean TP is unchanged but restricted
FP rises by two, and dim/bright/mean-robust C1 deltas are
`+0.011100/-0.003650/+0.003725`. Bright `0->1` falls only `95 -> 93`; six of ten
gates fail. Therefore no validation, test, XAI, mobile or INT8 promotion is
authorized.

A read-only, fixed `0.5/0.5` logit interpolation was computed only to localize
the failure; no coefficient was swept and no model was promoted. It yields
clean/dim/bright C1 `0.740385/0.669421/0.533333`, with clean restricted FP `33`
and bright `0->1=76`. Thus the endpoints are complementary in prediction space,
whereas elementwise weight averaging destroys part of the robust direction.
This two-checkpoint result is not mobile-deployable and may not be reported as a
B41 pass. A successor must test one equation-distinct, single-checkpoint merge
on a fresh TRAIN fold and must first establish that parameter sign conflict is
large enough to motivate it; ordinary model soup on this fold remains closed.

Formal artifact
`runs/b41_backbone_routed_pair_mean_fold3_fbea170_20260809/summary.json` has
SHA256
`8f8bdaa170755ef9ed2761fb56ceca16dd18b97dabfda8d8c1d1722e19ed03b8`; its
locked score archive has SHA256
`6c4488406fdbc9f42a2157bd3ed23ebd2450c1dc4f12ea46736004d303fa59ee`.
...
The final repaired preflight on commit `db41aef` passes every gate. Exact
midpoint state, direct-block function replay and post-training materialization
errors are all zero; MSE falls from `0.759687` to `0.618606` (`0.814291x`),
coefficient extrema move by `0.266488`, RNG is unchanged, and peak CUDA
allocation is `0.379815 GiB`. The separately retained `2.74658e-4` hooked
full-model-context difference is deterministic CUDA instance-level numerical
variation, not the direct saved-block replay used by the locked `1e-5` gate.
Artifact `runs/b43_progressive_merge_preflight_db41aef_20260809/preflight.json`
has SHA256
`2d9f183465ad5e7ab80b80aadf095ed54cdbac3dfc147f569c27391096eabd86`.

The authorized formal screen is now fixed. Fresh direct and backbone-routed
endpoints are trained on component-fold-4 fit rows with the exact B41 nine-epoch
schedule, seed `42`, auxiliary start at epoch `3` and weight `0.20`. Calibration
uses the fold-4 fit side only: the same deterministic `16` base images per class
provide `80` clean control views and `80` routed-teacher views, split into
`40` dim and `40` bright. Non-block parameters start at the exact arithmetic
midpoint. Blocks `0..11` are consolidated in order; each block learns only its
two elementwise task-vector coefficient tensors for `10` fixed-order epochs,
Adam `0.01`, with each batch containing eight clean and eight robust rows.
Every layer must have finite non-increasing activation MSE, exact midpoint state
within `1e-5`, materialized replay within `1e-5`, and total peak CUDA allocation
at most `6 GiB`. The resulting ordinary DINO state must replay exactly after
save/load and is evaluated once on held TRAIN fold 4 under B41's unchanged
