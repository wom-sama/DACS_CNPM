...
- Lock B16 SwiftSurface-XS as the only successor: a 3.062M-parameter mobile student with raw-DINO fit-only relation supervision and a spatial-residual versus mean-broadcast control. It has no TRAIN authority until its label-free preflight passes.
...
| `B16-01` | `LOCKED` | A hybrid should be rescued by another unconstrained branch/backbone sweep, or raw pretraining alone proves the design. | Rejected. Commit `abcb6ad48f1e9522a7b6f949d4e5d71e941eb528` locks one different causal test: SwiftFormer-XS S2 local residual versus an equal-state mean-broadcast control, both sharing a projector-free 364-edge raw-DINO fit-only target. Protocol SHA256 `4184395eb2c599d5bbd8b0cfa0a44cd20a73864b2b509f8602b0c73573c99cde`; no asset, TRAIN, validation or test evidence has been opened. | Only an accepted offline label-free provenance/CUDA/ONNX/QDQ/latency preflight may authorize the one five-fold TRAIN-only screen. Any scientific/engineering gate failure closes the exact route; a pass still requires a new validation/device protocol. |
| `CLEAN-01` | `CLOSED` | Old pretrained runs can be deleted by name or age. | Rejected. Tier A/B removed only reviewed obsolete roots. Tier C preserved 73 hashed evidence copies, protected 29 B9/B13/B14/B15/ledger anchors, and deleted exactly 23 superseded descriptor/checkpoint files (`15,584,026,418` bytes). | Any Tier D cleanup needs a new exact manifest and independent verification; completed tiers never authorize name/age/wildcard deletion. |
