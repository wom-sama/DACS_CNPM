...
- Close exact B17 SurfaceFold-XS at its first formal preflight on commit `954655c`. The run failed before deployment and before data because `torch.manual_seed` changed the already-initialized CUDA RNG inside `fork_rng(devices=[])`. Prepare one B18 protocol-only successor that preserves the exact architecture/training hypothesis while fixing GPU RNG isolation and using a target-matched AMD64 host artifact; it is not yet authorized to train.
