...
| `DEPLOY-01` | `OPEN` | Host ORT QDQ latency is a valid proxy for mobile efficiency and quantization must be faster. | Not established. On the locked four-thread CPUExecutionProvider audit, all three fully covered QDQ graphs were about `2.4--2.5x` slower than their FP32 graphs, while the 26,092-parameter branch added only about `3%` p95 overhead. The failure is backend/target-specific, not a candidate-only cost. | Before another architecture metric is opened, freeze a backend/device qualification that tests the stock graph first and records absolute plus paired relative latency; do not choose the backend or threshold from candidate results. |
