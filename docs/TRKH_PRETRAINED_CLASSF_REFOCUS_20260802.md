...
- Lock exactly one new backbone screen, B14 iFormer-S pooled-320, after source/checkpoint and synthetic deployment qualification. It is a released-representation comparison, not a causal hybrid-versus-ViT claim; no backup architecture or validation/test access is bundled with it.
...
| `B13-01` | `CLOSED_FAIL` | EfficientViM-M1 final feature is already a superior mobile transfer representation. | Rejected TRAIN-only: versus raw DINO, M1 final-320 loses mean pair AUROC `-0.014154`, macro-F1 `-0.032695` and C1 F1 `-0.042563`; every bootstrap interval is below zero. Mobile eligibility passes, but it cannot authorize fine-tuning. | No same-evidence reopen: the locked B13 stop rule forbids M2/M3/M4, four-stage fusion and readout replacement after seeing the result. A future canonical M1 interface requires fresh prospectively locked evidence. |
| `B14-01` | `LOCKED` | A compact local/global backbone can be selected by trying several pretrained families until one wins. | Rejected as a process. Exactly one hypothesis is locked: official iFormer-S v0.9, canonical pooled 320-D, compared with locked B13 DINO on immutable TRAIN folds. Source/checkpoint strict-load and synthetic ONNX/mobile eligibility pass; no TRAIN descriptor or metric has been computed. | Apply every preregistered gate. Failure closes this exact route without iFormer variants, stage/readout/input/fine-tune sweeps or a backup architecture; pass permits only a separate protocol. |
