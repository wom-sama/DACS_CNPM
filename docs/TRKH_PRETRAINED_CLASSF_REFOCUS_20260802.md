...
- Lock B17 SurfaceFold-XS as the single causal successor, pending a clean offline preflight. It adapts the frozen stage-3 downsample itself, receives relation supervision at that adapted output, uses a matched mean-surface control, and folds to exact stock topology. It is not yet a result and has no TRAIN/validation/test metric.
...
| `B17-01` | `LOCKED` | A lightweight hybrid should add measured local-direction information where its auxiliary objective can directly train the new mechanism, rather than append another free late branch. | SurfaceFold-XS uses `delta_W[o,i,h,w]=P[o,i]D[i,h,w]` in frozen SwiftFormer stage-3 downsample, raw-DINO relation loss on its post-BN output, and an equal-state mean-`D` control. Factors fold away to the exact 3,035,570-parameter stock topology. Independent protocol/code reviews passed; protocol/model/deployment SHA256 are `56ae72973b844186198dbc78c94851f48cf463b902370d2b3d2078b280169540`, `7707ed6153fc4beed91312fc5d962f84a65652468e428f7da680546f899778db` and `bd662257105a1852539cc37ae10da3d82600129c7e35433b726355ef64a7cb00`. No dataset or quality metric was opened. | Only a clean committed offline preflight may authorize one fixed TRAIN-only 5-fold/3-arm screen. Any implementation/gate failure closes exact B17; a TRAIN pass remains `DEVICE_PENDING` and cannot open validation before stock-first Android qualification. |
