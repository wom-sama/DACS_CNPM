...
| `B46-01` | `LOCKED` | B43 failed because activation matching never constrained the classifier boundary; one graph may instead learn clean-control logits and relit-robust logits under explicit class-1 margin protection. | Metric-free preflight reuses ten balanced fit-side B43 bases. The relit target keeps the robust teacher's complete non-target geometry and shifts only the true logit by the minimum needed to weakly dominate both teachers' true-class and `1<->0/2/4` margins. A control-initialized ordinary DINO gets 30 deterministic BF16 AdamW steps at `1e-5`; no checkpoint or held metric is produced. | Proceed to one fresh TRAIN-fold screen only if robust target loss falls to `<=0.85x`, clean loss stays `<=0.25x` initial robust loss, every clean argmax is retained, projection invariants pass and peak VRAM is `<=6 GiB`. Otherwise close without a metric run. |
