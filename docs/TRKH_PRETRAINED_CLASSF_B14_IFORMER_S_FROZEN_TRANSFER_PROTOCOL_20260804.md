...
The current timm `1.0.27` factory injects an incompatible `pretrained_cfg` argument into this v0.9 source. The runner must use the pinned direct constructor through a small compatibility adapter and must not alter the official model file. Preflight and formal execution are locked to Python `3.9.11`, NumPy `1.26.4`, PyTorch `2.6.0+cu124`, torchvision `0.21.0+cu124`, timm `1.0.27`, ONNX `1.19.1`, ONNX Runtime GPU distribution `1.19.2` using its CPU provider, and scikit-learn `1.6.1`; the committed adapter path and SHA are part of the accepted preflight.
...
- Locked B13 comparator root: `runs/pretrained_efficientvim_classf_b13_frozen_transfer_f229649_r1`. Require `summary.json` SHA256 `b017d34955f1c9126a611cf064cb919b6696ed9a19bf0f2b676838ad97955a75`, `train_oof_readouts.npz` SHA256 `82bcef6d293223b819f559122bffee822b7f2a2c80a200113dd6c0b7843def39`, and `train_paths.json` SHA256 `a472608c3d0b1ce1b941a1fa4bfa72b9f3032140d7f4c2792cb9bdd357b1c0fb`. Bind the unchanged B13 runner SHA256 `641791100aa626379637a2d939f7c36c7971779ab4b723ce40898d2a457fa7cb` for metric/bootstrap definitions.
- The comparator validator requires protocol ID `TRKH_PRETRAINED_CLASSF_B13_EFFICIENTVIM_M1_FROZEN_TRANSFER_20260803`, B13 HEAD `f229649fed92b56d3ec43f13bd1c23919d956abf`, `integrity_complete=true`, exact TRAIN-only flags, assignment-vector SHA256 `fca50be670fac7da20dd23cb382f9096c3c071d1a1e0dbaa1fd123e65c8aeb4b`, group-vector SHA256 `1243b61c91497459fa12036d1b9f63770ba4f1f11bb132ca17fb3b6013f1ade2`, exact labels/folds/groups/path order and complete finite DINO scores.
- Reuse only B13 labels, folds, groups, ordered paths and raw-DINO OOF scores. EfficientViM descriptors, scores and metrics are not B14 inputs; a negative test must fail if any code path requests the `efficientvim_scores` key.

Validation and test datasets, paths, images, predictions and metrics are forbidden. Hash the YAML bytes only; do not call a generic dataset/YAML loader. Resolve the locked B13 ordered paths directly beneath the fixed canonical root, require every normalized relative path to start with `train/`, and fail on any other split. The runner must positively record `validation_not_constructed=true` and `test_not_constructed=true`, and a focused negative test must reject a non-TRAIN path. Historical validation numbers are context only and cannot select B14.

## Canonical iFormer descriptor and readout

- Apply PIL EXIF transpose, convert to RGB, resize-short-side `256` with torchvision bicubic interpolation and `antialias=true`, center crop `224`, scale to FP32 tensor, then normalize with ImageNet mean `[0.485,0.456,0.406]` and standard deviation `[0.229,0.224,0.225]`.
- Use `eval()` FP32 inference, batch `32`, workers `0`. Run the exact four official downsample/stage pairs, then official adaptive global average pooling. The descriptor is the raw `320-D` tensor immediately before `model.classifier.classifier.bn`; the misleading official `forward_features()` returns logits and is forbidden as a descriptor API.
...
- The deployment skeleton first strict-loads the full 1000-class checkpoint, keeps the pretrained classifier BN affine parameters and running statistics, then replaces only `classifier.classifier.l`. With seed `20260804`, initialize the new `320 -> 5` weight using the official truncated normal with standard deviation `0.02` and zero bias. The resulting graph must have exactly `6,243,973` parameters; replacing the whole classifier/BN is forbidden.
...
Report OOF accuracy, macro-F1, per-class precision/recall/F1 with `zero_division=0`, class-1 precision/recall/F1, restricted `0/2/4 -> 1` false positives divided by every true row in classes `{0,2,4}`, mean AUROC over `1-vs-{0,2,4}` using the decision margin `score_1 - score_rival`, all five folds and confusion matrices. Gates consume unrounded float64 values.

Use exactly `5000` paired fold-stratified whole-union-component bootstrap replicates with seed `20260803`; never resample individual rows. Use percentile quantiles `0.025/0.975`. The draw vector must reproduce B13 SHA256 `d8967cbc13b17b78dd225841bd82a1a3074a7f57c3728fc6dd600aba646ec469` before computing candidate-minus-DINO intervals.
...
- opset-17 ONNX with its byte size reported, using only standard `ai.onnx`/empty operator domains;
- ONNX Runtime CPU argmax exactly equal to PyTorch and maximum absolute error at most `1e-5` for both graphs;
- four threads, batch one, 15 warmups and five alternating 100-run trials on this machine, retaining the pre-existing B13 thresholds: iFormer median latency no more than `0.25x` DINO and p95 no more than `0.30x` DINO;
- finite official forward, exact pooled-feature reconstruction and no source/checkpoint drift.

The initial diagnostic qualification observed a parameter ratio `0.28922`, standard-domain ONNX with `25,032,211` bytes, parity error `1.67e-6`, median ratio `0.17965` and p95 ratio `0.17089`. This is a compatibility result only and does not satisfy formal preflight: the committed runner must independently reproduce and hash it under the stricter existing thresholds. The official `0.85 ms` iPhone 13/CoreML result is external eligibility evidence, not local phone certification. Android, INT8/FP16 accuracy, thermal latency and memory remain unaudited.

## Stop rule and artifacts

Commit and hash the protocol and synthetic preflight before extracting descriptors. Formal execution requires that exact preflight SHA, a clean tracked worktree, a clean pinned source checkout and an unused output directory. Persist dependency versions, source/release/checkpoint hashes, permissions, descriptor/path/OOF hashes, readout state, bootstrap draw hash, metrics, gates, wall time, peak allocated VRAM, output bytes and start/end TRAIN integrity. The operational budget is 15 minutes wall time, 6 GiB peak allocated GPU memory and 100 MiB formal artifacts; exceeding it is an infrastructure failure, never an accuracy result.

Source/comparator/preflight mismatch, download failure, crash or OOM before metric construction makes a run invalid rather than a scientific failure. One infrastructure retry is allowed only at the identical commit, protocol, environment and arguments, without inspecting partial descriptors/scores; changing batch size or architecture is forbidden. Write through a sibling `.partial` directory, quarantine any failed partial root with a hash manifest, and never delete it except through a reviewed cleanup manifest. Once candidate metrics exist, the registered gate is final and no retry may select a different result.

If the synthetic gate fails scientifically, close before descriptor access. If any representation gate fails, close the exact official iFormer-S-v0.9 / pooled-320 / 224-pixel / balanced-linear-readout route. Do not then try iFormer-T/M/L, the CoreML package, classifier logits, other stages, concatenation, alternate pooling, input size, readout C, loss, fine-tuning or distillation on this evidence. Do not substitute PoolFormer or SHViT inside B14.

Passing authorizes only one separately committed fine-tuning protocol. It does not authorize validation, test, full train, XAI claims, model comparison publication, checkpoint/model-package redistribution or deployment promotion. Retain the accepted preflight, descriptor, OOF and summary artifacts until a reviewed cleanup manifest says otherwise. Human blinded review of the unresolved class-1 queue remains an independent requirement.
