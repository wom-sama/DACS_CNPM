...
param(
    ...
    [string]$OutputDir = "D:\DataAI\AIEx\TRKH\runs\audit_deepbdc_stem_joint_dependence_a0_20260721",
...
$Module = "trkh.tools.audit_deepbdc_stem_joint_dependence_a0"
$FocusedTest = "tests\test_audit_deepbdc_stem_joint_dependence_a0.py"
...
function Show-ResourceState {
    Write-Host "DeepBDC host process snapshot (read-only):"
...
function Assert-PushedTrackedState {
    ...
    if (-not [string]::IsNullOrWhiteSpace($tracked)) {
        throw "Formal DeepBDC A0 requires a clean tracked worktree. Commit/push the isolated auditor first."
    }
    $head = (& git -C $RepoRoot rev-parse HEAD).Trim()
    $upstream = (& git -C $RepoRoot rev-parse '@{upstream}').Trim()
    if ($LASTEXITCODE -ne 0 -or $head -ne $upstream) {
        throw "Formal DeepBDC A0 requires HEAD to equal its upstream."
    }
}
...
function Invoke-StaticPreflight {
    ...
    Invoke-PythonChecked -Arguments @(
        "-m", "py_compile",
        "trkh\tools\audit_deepbdc_stem_joint_dependence_a0.py"
    )
    Invoke-PythonChecked -Arguments @(
        "-m", "pyflakes",
        "trkh\tools\audit_deepbdc_stem_joint_dependence_a0.py",
...
function Invoke-FormalAudit {
    ...
    $env:TRKH_DEEPBDC_A0_PREFLIGHT = "passed"
    $env:PYTHONHASHSEED = "20260721"
    ...
    try {
        ...
        Remove-Item Env:TRKH_DEEPBDC_A0_PREFLIGHT -ErrorAction SilentlyContinue
...
function Invoke-ExternalReplay {
    if (-not (Test-Path -LiteralPath $SummaryPath -PathType Leaf)) {
        throw "DeepBDC summary is missing: $SummaryPath"
    }
    Invoke-PythonChecked -Arguments @(
        "-m", $Module,
...
function Invoke-VisualFinalization {
    ...
    if ([string]::IsNullOrWhiteSpace($ExpectedSummarySha256)) {
        throw "Visual finalization requires -ExpectedSummarySha256 from the replay output."
    }
    Invoke-PythonChecked -Arguments @(
        "-m", $Module,
        "--replay-summary", $SummaryPath,
