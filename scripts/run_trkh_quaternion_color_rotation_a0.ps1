...
$Module = "trkh.tools.audit_quaternion_color_rotation_a0"
$FocusedTest = "tests\test_audit_quaternion_color_rotation_a0.py"
...
function Show-ResourceState {
    Write-Host "Quaternion A0 host process snapshot (read-only):"
...
function Assert-PushedTrackedState {
    ...
    if (-not [string]::IsNullOrWhiteSpace($tracked)) {
        throw "Formal quaternion A0 requires a clean tracked worktree. Commit and push the isolated auditor first."
    }
    $head = (& git -C $RepoRoot rev-parse HEAD).Trim()
    $upstream = (& git -C $RepoRoot rev-parse '@{upstream}').Trim()
    if ($LASTEXITCODE -ne 0 -or $head -ne $upstream) {
        throw "Formal quaternion A0 requires HEAD to equal its upstream."
    }
}
...
function Invoke-StaticPreflight {
    ...
    Invoke-PythonChecked -Arguments @(
        "-m", "py_compile",
        "trkh\tools\audit_quaternion_color_rotation_a0.py"
    )
    Invoke-PythonChecked -Arguments @(
        "-m", "pyflakes",
        "trkh\tools\audit_quaternion_color_rotation_a0.py",
...
function Invoke-FormalAudit {
    ...
    $env:TRKH_QUATERNION_A0_PREFLIGHT = "passed"
    ...
    try {
        ...
        Remove-Item Env:TRKH_QUATERNION_A0_PREFLIGHT -ErrorAction SilentlyContinue
...
function Invoke-ExternalReplay {
    if (-not (Test-Path -LiteralPath $SummaryPath -PathType Leaf)) {
        throw "Quaternion A0 summary is missing: $SummaryPath"
    }
    Invoke-PythonChecked -Arguments @(
        "-m", $Module,
