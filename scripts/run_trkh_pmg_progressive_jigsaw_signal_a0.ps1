param(
    ...
    [ValidateSet("Preflight", "Formal", "Replay", "VisualPass", "VisualFail")]
    [string]$Phase = "Preflight",
    [string]$OutputDir = "runs\audit_pmg_progressive_jigsaw_signal_a0_20260720",
...
function Invoke-LockedPython {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "PMG progressive-jigsaw command failed with exit code ${LASTEXITCODE}: $($Arguments -join ' ')"
    }
}
...
function Assert-GpuIsolation {
    ...
    if ($Processes) {
        $Processes | Format-Table -AutoSize | Out-Host
        throw "Formal PMG A0 requires no pre-existing Python/TensorRT/FFmpeg process."
    }
    ...
    if (($MaximumUtilization -gt 20) -and (-not $LowPowerIdle)) {
        throw "Formal PMG A0 is blocked by non-idle background GPU activity."
    }
...
if ($Phase -in @("Preflight", "Formal")) {
    Invoke-LockedPython -Arguments @(
        "-m", "py_compile",
        "trkh\tools\audit_pmg_progressive_jigsaw_signal_a0.py",
        "tests\test_audit_pmg_progressive_jigsaw_signal_a0.py"
    )
    Invoke-LockedPython -Arguments @(
        "-m", "pyflakes",
        "trkh\tools\audit_pmg_progressive_jigsaw_signal_a0.py",
...
if ($Phase -eq "Preflight") {
    ...
    Show-ComputeState
    Invoke-LockedPython -Arguments @(
        "-m", "trkh.tools.audit_pmg_progressive_jigsaw_signal_a0",
        "--preflight-only",
    ...
    Write-Host "PMG A0 preflight passed without model/dataset loading or output creation."
    exit 0
}
...
if ($Phase -in @("VisualPass", "VisualFail")) {
    if (-not (Test-Path -LiteralPath $SummaryPath -PathType Leaf)) {
        throw "PMG A0 summary is missing: $SummaryPath"
    }
...
if (Test-Path -LiteralPath $OutputDir) {
    throw "Formal PMG A0 output already exists; rerun is forbidden: $OutputDir"
}
Show-ComputeState
...
try {
    ...
    Write-Host "PMG progressive-jigsaw A0 status: $($Summary.status)"
