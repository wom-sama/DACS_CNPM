param(
    ...
    [string]$OutputDir = "runs\audit_spatial_worst_shift_robust_optimization_a0_20260717"
)
...
function Assert-IsolatedRuntime {
    ...
    if ($Unexpected) {
        ...
        throw "Spatial A0 found another Python/TensorRT process; none was terminated.`n$Details"
    }
    $GpuRows = & nvidia-smi --query-gpu=utilization.gpu,memory.used --format=csv,noheader,nounits
    if ($LASTEXITCODE -ne 0 -or -not $GpuRows) {
        throw "Khong doc duoc nvidia-smi cho Spatial A0."
    }
...
Invoke-LockedPython -Arguments @(
    "-m", "py_compile",
    "trkh\tools\audit_spatial_worst_shift_robust_optimization_a0.py",
    "tests\test_audit_spatial_worst_shift_robust_optimization_a0.py"
)
Invoke-LockedPython -Arguments @(
    "-m", "pytest",
    "tests\test_audit_spatial_worst_shift_robust_optimization_a0.py",
...
if ($Phase -eq "Preflight") {
    ...
    Invoke-LockedPython -Arguments @(
        "-m", "trkh.tools.audit_spatial_worst_shift_robust_optimization_a0",
        "--preflight-only"
    )
    Write-Host "Spatial worst-shift A0 preflight passed without creating output."
    exit 0
}
...
if (-not (Test-Path -LiteralPath $SummaryPath -PathType Leaf)) {
    throw "Spatial A0 khong tao summary: $SummaryPath"
}
$SummarySha = (Get-FileHash -Algorithm SHA256 -LiteralPath $SummaryPath).Hash.ToLowerInvariant()
$Summary = Get-Content -Raw -LiteralPath $SummaryPath | ConvertFrom-Json
Write-Host "Spatial A0 completed with status: $($Summary.status)"
Write-Host "Summary SHA-256: $SummarySha"
Write-Host "Current-best command update remains forbidden at A0."
