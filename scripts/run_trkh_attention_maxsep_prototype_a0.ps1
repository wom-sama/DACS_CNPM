...
param(
    ...
    [string]$OutputDir = "D:\DataAI\AIEx\TRKH\runs\audit_attention_maxsep_prototype_a0_20260721",
...
$Module = "trkh.tools.audit_attention_maxsep_prototype_a0"
$FocusedTest = "tests\test_audit_attention_maxsep_prototype_a0.py"
...
function Show-ResourceState {
    Write-Host "Attention-MaxSep A0 host process snapshot (read-only):"
...
function Assert-PushedTrackedState {
    ...
    if (-not [string]::IsNullOrWhiteSpace($tracked)) {
        throw "Formal Attention-MaxSep A0 requires a clean tracked worktree."
    }
    $head = (& git -C $RepoRoot rev-parse HEAD).Trim()
    $upstream = (& git -C $RepoRoot rev-parse '@{upstream}').Trim()
    if ($LASTEXITCODE -ne 0 -or $head -ne $upstream) {
        throw "Formal Attention-MaxSep A0 requires HEAD to equal its upstream."
    }
    $requiredFiles = @(
    ...
    foreach ($requiredFile in $requiredFiles) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Formal Attention-MaxSep A0 requires tracked file: $requiredFile"
        }
    }
}
...
function Invoke-StaticPreflight {
    ...
    Invoke-PythonChecked -Arguments @(
        "-m", "py_compile",
        "trkh\tools\audit_attention_maxsep_prototype_a0.py",
        $FocusedTest
    )
    Invoke-PythonChecked -Arguments @(
        "-m", "pyflakes",
...
function Invoke-FormalAudit {
    ...
    $env:TRKH_ATTENTION_MAXSEP_A0_PREFLIGHT = "passed"
    ...
    try {
        ...
        Remove-Item Env:TRKH_ATTENTION_MAXSEP_A0_PREFLIGHT -ErrorAction SilentlyContinue
...
function Invoke-ExternalReplay {
    if (-not (Test-Path -LiteralPath $SummaryPath -PathType Leaf)) {
        throw "Attention-MaxSep A0 summary is missing: $SummaryPath"
    }
    Invoke-PythonChecked -Arguments @(
        "-m", $Module,
