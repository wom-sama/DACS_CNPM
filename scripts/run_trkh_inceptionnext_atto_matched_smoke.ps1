...
if ($stageA.method -ne "inceptionnext_atto_surface_tokenizer") {
    throw "Stage-A method does not match InceptionNeXt-Atto."
}
if (-not [bool]$stageA.gate.smoke_permission) {
    throw "Stage-A InceptionNeXt gate did not grant smoke permission."
}
...
$candidateArguments = New-LockedTrainArguments @($source.train_args) $CandidateRunName "inceptionnext_atto_tokenizer"
...
$protocol = [ordered]@{
    ...
    candidate_stem_architecture = "inceptionnext_atto_tokenizer"
...
$evaluationBase = @(
    ...
    "--family", "TRKH-InceptionNeXt-Atto-smoke"
)
...
Invoke-NativePython "evaluate_candidate_val" ($evaluationBase + @(
    ...
    "--paper-name", "TRKH-InceptionNeXt-Atto-120b-2e",
...
Invoke-NativePython "compare_smoke_pair" @(
    "-m", "trkh.tools.audit_inceptionnext_atto_smoke_pair",
...
$status | ConvertTo-Json | Set-Content -LiteralPath $statusPath -Encoding UTF8
Write-Host "Matched InceptionNeXt-Atto smoke pair completed: $PairOutputDir"
