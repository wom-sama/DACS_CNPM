param(
    ...
    [string]$Protocol = "docs\TRKH_5CLASS_VIG_MAX_RELATIVE_GRAPH_READINESS_PROTOCOL_20260715.md",
    [string]$OfficialVigRoot = "C:\Users\ADMIN\AppData\Local\Temp\trkh_next_arch_sources_20260715\vision_gnn",
    [string]$OutputDir = "",
    [int]$NumWorkers = 4,
    [switch]$Preflight
)
...
try {
    if ([string]::IsNullOrWhiteSpace($OutputDir)) {
        ...
        $OutputDir = "runs\audit_vig_graph_readiness_$stamp"
    }
    $Arguments = @(
        "-m", "trkh.tools.audit_vig_max_relative_graph_readiness",
        ...
        "--protocol", $Protocol,
        "--official-vig-root", $OfficialVigRoot,
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "ViG graph Stage-A audit failed with exit code $LASTEXITCODE."
    }
}
finally {
    Pop-Location
