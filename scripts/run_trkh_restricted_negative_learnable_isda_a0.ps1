param(
    ...
    [string]$OutputDir = "runs\audit_rn_lisda_a0_20260724",
    [string]$ReviewNote = "",
...
function Invoke-LockedPython {
    ...
    # Keep native stdout/stderr attached to the VS Code terminal. Redirecting
    # stderr through the PowerShell object pipeline creates NativeCommandError
    # records for valid Python INFO logs.
    & $Python @Arguments
    $ExitCode = $LASTEXITCODE
    if ($ExitCode -ne 0) {
        throw "RN-LISDA A0 failed with exit code ${ExitCode}: $($Arguments -join ' ')"
    }
}
...
function Assert-PowerShellParse {
    ...
    if ($ParseErrors.Count -gt 0) {
        ...
        throw "RN-LISDA launcher has PowerShell parse errors."
    }
}
...
Invoke-LockedPython -Arguments @(
    "-m", "py_compile",
    "trkh\tools\restricted_negative_learnable_isda_a0_engine.py",
    "trkh\tools\audit_restricted_negative_learnable_isda_a0.py",
    "tests\test_restricted_negative_learnable_isda_a0_engine.py",
    "tests\test_restricted_negative_learnable_isda_a0_auditor.py"
)
Invoke-LockedPython -Arguments @(
    "-m", "pyflakes",
    "trkh\tools\restricted_negative_learnable_isda_a0_engine.py",
...
if ($Phase -eq "Preflight") {
    Show-ComputeState
    Invoke-LockedPython -Arguments @(
        "-m", "trkh.tools.audit_restricted_negative_learnable_isda_a0",
        "--preflight-only"
    )
    exit 0
}
...
if ($Existing) {
    ...
    throw "Formal RN-LISDA A0 requires no pre-existing Python/TensorRT process."
}
$OperatingSystem = Get-CimInstance Win32_OperatingSystem
$AvailableGiB = ([double]$OperatingSystem.FreePhysicalMemory * 1KB) / 1GB
if ($AvailableGiB -lt 4.0) {
    throw "Formal RN-LISDA A0 requires at least 4 GiB available physical memory."
}
