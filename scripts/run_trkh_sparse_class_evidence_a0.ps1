...
param(
    ...
    [string]$OutputDir = "D:\DataAI\AIEx\TRKH\runs\audit_sparse_class_evidence_a0_20260721",
...
$Module = "trkh.tools.audit_sparse_class_evidence_a0"
$FocusedTest = "tests\test_audit_sparse_class_evidence_a0.py"
...
function Show-ResourceState {
    Write-Host "Sparse class-evidence host process snapshot (read-only):"
...
function Assert-PushedTrackedState {
    ...
    if (-not [string]::IsNullOrWhiteSpace($tracked)) {
        throw "Formal sparse class-evidence A0 requires a clean tracked worktree."
    }
    $head = (& git -C $RepoRoot rev-parse HEAD).Trim()
    $upstream = (& git -C $RepoRoot rev-parse '@{upstream}').Trim()
    if ($LASTEXITCODE -ne 0 -or $head -ne $upstream) {
        throw "Formal sparse class-evidence A0 requires HEAD to equal its upstream."
    }
    $requiredFiles = @(
    ...
    foreach ($requiredFile in $requiredFiles) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Formal sparse class-evidence A0 requires tracked file: $requiredFile"
        }
    }
}
...
function Invoke-StaticPreflight {
    ...
    Invoke-PythonChecked -Arguments @(
        "-m", "py_compile",
        "trkh\tools\audit_sparse_class_evidence_a0.py",
        $FocusedTest
    )
    Invoke-PythonChecked -Arguments @(
        "-m", "pyflakes",
...
function Invoke-FormalAudit {
    ...
    $env:TRKH_SPARSE_EVIDENCE_A0_PREFLIGHT = "passed"
    ...
    try {
        ...
        Remove-Item Env:TRKH_SPARSE_EVIDENCE_A0_PREFLIGHT -ErrorAction SilentlyContinue
...
function Invoke-ExternalReplay {
    if (-not (Test-Path -LiteralPath $SummaryPath -PathType Leaf)) {
        throw "Sparse class-evidence summary is missing: $SummaryPath"
    }
    Invoke-PythonChecked -Arguments @(
        "-m", $Module,
