param(
    ...
    [ValidateSet("Preflight", "Engineering", "Formal", "Replay", "VisualPass", "VisualFail")]
    [string]$Phase = "Preflight",
    [string]$OutputDir = "runs\audit_hamburger_nmf_surface_a0_20260720",
...
function Invoke-LockedPython {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Hamburger-NMF A0 command failed with exit code ${LASTEXITCODE}: $($Arguments -join ' ')"
    }
}
...
function Assert-PowerShellParse {
    ...
    if ($ParseErrors.Count -gt 0) {
        ...
        throw "Hamburger-NMF A0 launcher has PowerShell parse errors."
    }
}
...
function Assert-GpuIsolation {
    ...
    if ($Processes) {
        $Processes | Format-Table -AutoSize | Out-Host
        throw "Formal Hamburger-NMF A0 requires no pre-existing Python/TensorRT/FFmpeg process."
    }
    ...
    if (($MaximumUtilization -gt 20) -and (-not $LowPowerIdle)) {
        throw "Formal Hamburger-NMF A0 is blocked by non-idle background GPU activity."
    }
    if ($MaximumUtilization -gt 20) {
        Write-Host "Utilization exceeded 20 percent, but the low-power P8 idle fallback passed."
    }
}
...
function Invoke-StaticPreflight {
    Invoke-LockedPython -Arguments @(
        "-m", "py_compile",
        "trkh\tools\audit_hamburger_nmf_surface_a0.py",
        "tests\test_audit_hamburger_nmf_surface_a0.py"
    )
    Invoke-LockedPython -Arguments @(
        "-m", "pyflakes",
        "trkh\tools\audit_hamburger_nmf_surface_a0.py",
...
if ($Phase -in @("Preflight", "Engineering", "Formal")) {
    if (Test-Path -LiteralPath $OutputDir) {
        throw "Preflight must not create or reuse output: $OutputDir"
    }
    Invoke-StaticPreflight
    if (Test-Path -LiteralPath $OutputDir) {
        throw "Static preflight unexpectedly created output: $OutputDir"
    }
}
...
if ($Phase -eq "Preflight") {
    Show-ComputeState
    Write-Host "Hamburger-NMF A0 preflight passed without model or dataset loading."
    exit 0
}
...
if ($Phase -in @("VisualPass", "VisualFail")) {
    if (-not (Test-Path -LiteralPath $SummaryPath -PathType Leaf)) {
        throw "Hamburger-NMF A0 summary is missing: $SummaryPath"
    }
...
$OldAttestation = $env:TRKH_HAMBURGER_NMF_A0_PREFLIGHT
try {
    ...
    $env:TRKH_HAMBURGER_NMF_A0_PREFLIGHT = "passed"
    Invoke-LockedPython -Arguments @(
        "-m", "trkh.tools.audit_hamburger_nmf_surface_a0",
        "--engineering-forward",
        "--output-dir", $OutputDir,
    ...
    Write-Host "Hamburger-NMF A0 status: $($Summary.status)"
    Write-Host "Candidate AUROC: $($Summary.clean_analysis.metrics.nmf_candidate.auroc)"
    Write-Host "Candidate class-1 TP retention: $($Summary.clean_analysis.metrics.nmf_candidate.tp_retention)"
    Write-Host "Candidate restricted-FP rejection: $($Summary.clean_analysis.metrics.nmf_candidate.fp_rejection)"
    Write-Host "Stage B executed/passed: $($Summary.stage_b_executed)/$($Summary.stage_b_passed)"
    ...
    $env:TRKH_HAMBURGER_NMF_A0_PREFLIGHT = $OldAttestation
    if ($WallpaperPaused) {
