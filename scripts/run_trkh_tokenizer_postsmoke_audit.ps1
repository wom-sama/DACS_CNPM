param(
    ...
    [string]$LockedPairSummarySha256 = "163d4d1c2ba522712a1e7f16f9d89d5b9b82114e0760781ccb8f08694ba0d8bb",
    [string]$LockedControlCheckpointSha256 = "9ce2e6fdcf506be6066ec67ece2e37c2f6edb69dd7f403f193fc6e5044791bb1",
    [string]$LockedCandidateCheckpointSha256 = "cfe1050ac98549f5194fbb6cd6a0c2d42f34169d25e6881eabfa6dd20ca7d101",
    [string]$LockedCandidateTraceSha256 = "51387302d477dafb3e89f5d1a69cd9862796254e2457f2288c4d4b5c6b2e99aa",
...
if ($pair.method -ne $ExpectedMethod) {
    throw "Matched-smoke method does not match the locked tokenizer method."
}
...
$protocol = [ordered]@{
    method = "$ExpectedMethod`_postsmoke"
...
Invoke-NativePython "audit_candidate_trace" @(
    "-m", "trkh.tools.audit_tokenizer_trace",
...
Invoke-NativePython "forensics_candidate" ($forensicsBase + @(
    ...
    "--image-stats-cache", (Join-Path $ForensicsControlDir "predictions_forensics.csv"),
...
$XaiCohortSummary = Get-Content -Raw -LiteralPath (Join-Path $XaiCohortDir "summary.json") | ConvertFrom-Json
$XaiCaseCount = [int]$XaiCohortSummary.selected_case_count
if ($XaiCaseCount -lt 1 -or $XaiCaseCount -gt 16) {
    throw "XAI cohort size is outside the locked range 1..16: $XaiCaseCount"
}
$xaiBase = @(
...
Invoke-NativePython "paired_xai" @(
    ...
    "--expected-cases", ([string]$XaiCaseCount),
...
Invoke-NativePython "postsmoke_closure" @(
    "-m", "trkh.tools.audit_tokenizer_postsmoke",
...
Save-Status $status $StatusPath
Write-Host "Tokenizer post-smoke audit completed: $PostsmokeAuditDir"
